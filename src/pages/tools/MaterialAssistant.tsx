...
export default function MaterialAssistant() {
  const [messages, setMessages] = useState<MessageNode[]>([
    {
      id: "msg_1",
      role: "assistant",
      text: "お疲れ様です！AI材料アシスタントの『ケンサック』です。\n「H鋼に配管したい」「この壁面に固定したい」など、現場の状況をテキストや写真でポンと教えてください。\nデータベースの中から最適な支持金具を探し出してサクッとご提案します！",
    }
  ]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const systemPrompt = `あなたは建設・電気工事の職人さんを頼もしくサポートするAI現場相棒「ケンサック」です。
以下の【自社でよく使用する材料カタログ】の情報のみを使用して回答してください。
...
【会話のトーンとルール】
- 自分の名前（「ケンサックです」等）は冗長になるので都度は名乗らず、すぐに回答に入ってください。
- ユーザーを「職人さん」と呼ぶのではなく、現場の気のいい相棒のように「お疲れ様です！」等の声かけにしてください。
