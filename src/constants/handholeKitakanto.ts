...
/**
 * 配管が直進して反対側から抜ける場合の相手面（対辺）。2026-09-18、社長の実務知見「ハンドホールは
 * 直線に使う場合が多いので、A面に使ったらC面、B面に使ったらD面という風に直線上に抜けていく」を
 * そのまま採用。
 *
 * 【根拠の限界】北関東工業のDXF実物解析（KKE_450_FACE_DXF_ORIGIN）では、シート上でA/B面が同じ
 * Y座標（横に並ぶ）・C/D面が同じY座標（横に並ぶ）・A/C面が同じX座標（縦に並ぶ）ことは実測済みだが、
 * これは「図面用紙上の並べ方」であり、実物の箱を4面時計回りにA→B→C→Dと呼ぶ命名規則そのものを
 * 直接証明するものではない（北関東工業の資料に時計回り/反時計回りの明記は無い）。実際に現場で
 * 使われた削孔図16基（オーイケ製、2026-09-18調査）でも「A面は常に無孔」「B/D面またはC/D面に
 * 集中」という、面ごとの使い方に偏りがある実例が確認できており、対辺の考え方自体は実務と整合する。
 * 将来メーカー資料で矛盾する記載が見つかった場合は要修正。
 */
export const FACE_OPPOSITE: Record<HandholeFace, HandholeFace> = { A: 'C', B: 'D', C: 'A', D: 'B' };
