...
import { Settings, Menu, Bell, ClipboardList, LayoutDashboard, FileText, CheckSquare, CalendarClock, CalendarDays, PieChart, ShieldAlert, Truck, FileSignature, Wrench, Bot, Thermometer, Zap, LogOut, Cable, Contact, Video, Target, Boxes } from "lucide-react"
  ...
  return (
    <div className="h-screen w-full bg-background text-foreground flex flex-col overflow-hidden">
      ...
      <div className="flex-1 flex overflow-hidden relative">
          ...
          <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
            {appMode === 'tools' && (
              <>
                ...
                {/* ハンドホール穴あけ（独立ページ） */}
                <NavLink to="/handhole-knockout" className={getNavClass}>
                  {({ isActive }) => (
                    <>
                      <Boxes className={`w-5 h-5 ${isActive ? 'text-blue-600' : 'text-slate-500 group-hover:text-slate-700'}`} />
                      ハンドホール穴あけ（北関東工業）
                      {isActive && <span className="absolute left-0 top-0 bottom-0 w-1.5 bg-blue-600 rounded-r-md"></span>}
                    </>
                  )}
                ...
                {/* 作業手順動画（独立ページ） */}
                <NavLink to="/training-videos" className={getNavClass}>
                  {({ isActive }) => (
                    <>
                      <Video className={`w-5 h-5 ${isActive ? 'text-blue-600' : 'text-slate-500 group-hover:text-slate-700'}`} />
                      作業手順動画
                      {isActive && <span className="absolute left-0 top-0 bottom-0 w-1.5 bg-blue-600 rounded-r-md"></span>}
                    </>
                  )}
            ...
            <NavLink to="/customers" className={getNavClass}>
              {({ isActive }) => (
                <>
                  <Contact className={`w-5 h-5 ${isActive ? 'text-blue-600' : 'text-slate-500 group-hover:text-slate-700'}`} />
                  顧客管理
                  {isActive && <span className="absolute left-0 top-0 bottom-0 w-1.5 bg-blue-600 rounded-r-md"></span>}
                </>
              )}
            ...
            {/* AIチャットはダッシュボード方針への転換により非表示化(ページ・Edge Functionは削除せず温存) */}
            {isAdmin && (
        ...
        {/* Page Content */}
        <main className="flex-1 p-4 sm:p-6 md:p-8 overflow-y-auto overflow-x-hidden flex flex-col min-w-0 w-0">
