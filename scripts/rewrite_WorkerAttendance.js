...
const FILE_PATH = 'src/pages/attendance/WorkerAttendance.tsx';
...
export default function WorkerAttendance() {
  ...
  const openModal = (dateStr: string) => {
    ...
    if (existingRecord) {
        ...
        // Reverse engineer remaining events from site declarations
        // Wait, for perfect UX, we just place the site declarations in order and travel blocks.
        const decls = [...(existingRecord.site_declarations || [])].sort((a,b) => (a.start_time || '').localeCompare(b.start_time || ''));
        
        // Add travels in between gaps? The user might have just saved it.
        // Actually, if we just list the site starts:
        let lastEnd = ci;
        for (const d of decls) {
             if (d.start_time) {
                 // if there is a gap > 1 minute from previous end, insert travel
                 if (lastEnd && lastEnd !== d.start_time) {
                     // actually, we just add site work directly
                 }
                 initialEvents.push({
        ...
        if (co) {
             // If there was a gap until clock out, maybe we need travel?
             // Since travel time was aggregate, it's hard to place it accurately if we didn't save timeline_events.
             // We will just put a travel block before clock out if prep_travel > 0 and no travel block
        ...
        // Template from assigned projects
  ...
  const saveRecord = async () => {
    ...
    // Validate times
    const sorted = [...timelineEvents].sort((a, b) => a.time.localeCompare(b.time));
    for (let i = 0; i < sorted.length - 1; i++) {
        if (sorted[i].time && sorted[i+1].time && sorted[i].time > sorted[i+1].time) {
             toast.error('入力された時間が逆転しています。正しく時系列に並べてください。');
             return;
        }
    }
    
    // Process timeline to DB fields
    ...
    // Sort events by time to process intervals
    const validEvents = timelineEvents.filter(e => e.time).sort((a,b) => a.time.localeCompare(b.time));
    ...
    // Calculate total private out minutes automatically
  ...
  const getEventBgColor = (type: string) => {
      switch(type) {
          case 'clock_in': return 'bg-amber-100 text-amber-800 border-amber-200';
          case 'clock_out': return 'bg-amber-100 text-amber-800 border-amber-200';
          case 'site_work': return 'bg-blue-100 text-blue-800 border-blue-200';
          case 'travel': return 'bg-slate-100 text-slate-800 border-slate-200';
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 w-full max-w-5xl mx-auto space-y-6">
      <div className="shrink-0 space-y-2 pb-2">
        ...
        <p className="text-muted-foreground">現場での役割、移動時間、準備時間を入力してください</p>
      </div>

      {!workerId && !loading && (
        <div className="bg-red-50 text-red-600 border border-red-200 p-4 rounded-lg flex items-center gap-2 font-bold shadow-sm">
          <span>⚠️ あなたのアカウントは作業員マスターに登録されていないため、自身の出退勤や役割の申告を行うことはできません。</span>
        </div>
      )}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-slate-100 rounded-xl shadow-xl w-full max-w-xl h-[90vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center p-4 border-b bg-white">
              <h3 className="font-bold text-lg flex flex-col">
                  <span>{selectedDate && new Date(selectedDate).toLocaleDateString('ja-JP', { month: 'short', day: 'numeric', weekday: 'short' })} の実績入力（タイムライン）</span>
                  <span className="text-sm font-normal text-muted-foreground mt-1">次の項目を入れると、そこまでの時間が自動計算されます</span>
              </h3>
              <button onClick={() => setIsModalOpen(false)} className="hover:bg-slate-200 p-1 rounded-full"><X className="w-5 h-5" /></button>
            </div>
            
            <div className="p-4 flex-1 overflow-y-auto space-y-6">
              
              <div className="relative border-l-2 border-slate-300 ml-6 pl-6 space-y-1">
                {timelineEvents.map((ev, idx) => {
                    ...
                    return (
                        <div key={ev.id} className="relative group">
                            {/* Timeline Node dot */}
                            <div className={\`absolute -left-[31px] top-3 w-4 h-4 rounded-full border-2 border-white \${ev.type === 'clock_out' ? 'bg-red-500' : 'bg-blue-500'}\`}></div>
                            
                            {/* Card Array */}
                            <div className={\`bg-white border rounded-xl shadow-sm p-3 flex flex-col gap-2 relative transition-all focus-within:ring-2 ring-blue-500 \${getEventBgColor(ev.type)}\`}>
                                <div className="flex items-center gap-3 w-full">
                                    <input 
                                       type="time" 
                                       value={ev.time}
                                       onChange={(e) => updateEventInfo(idx, { time: e.target.value })}
                                       className="w-28 h-9 rounded-md border text-center font-bold font-mono shadow-sm bg-white"
                                    />
                                    <span className="font-bold shrink-0">{ev.type === 'clock_out' ? 'に退勤した' : 'から'}</span>
                                    
                                    {ev.type !== 'clock_in' && ev.type !== 'clock_out' ? (
                                        <select 
                                           value={ev.type}
                                           onChange={(e) => updateEventInfo(idx, { type: e.target.value as any, project_id: '', project_name: '' })}
                                           className="flex-1 h-9 rounded-md border font-bold text-sm px-2 shadow-sm bg-white"
                                        >
                                           <option value="travel">移動する / 準備する</option>
                                           <option value="site_work">現場で作業する</option>
                                        </select>
                                    ) : (
                                        <span className="font-bold text-lg flex-1">
                                            {ev.type === 'clock_in' ? '出社（出勤）した🚀' : 'お疲れ様でした！🏠'}
                                        </span>
                                    )}

                                    {ev.type !== 'clock_in' && ev.type !== 'clock_out' && (
                                        <button onClick={() => removeEvent(idx)} className="p-2 text-slate-300 hover:text-red-500 rounded"><X className="w-5 h-5"/></button>
                                    )}
                                </div>

                                {isSite && (
                                    <div className="pl-[7rem] pr-2 pb-1 flex gap-2 w-full">
                                        <div className="flex-1">
                                            <select 
                                                className="w-full h-8 px-2 text-sm border rounded bg-white shadow-sm"
                                                value={ev.project_id || ''}
                                                onChange={(e) => {
                                            ...
                                            >
                                                ...
                                                {allProjects.map(p => <option key={p.id} value={p.id}>{p.project_name} {p.status === '完工' ? '(完工)' : ''}</option>)}
                                            </select>
                                        </div>
                                        <div className="w-28">
                                            <select 
                                                className="w-full h-8 px-2 text-sm border bg-white rounded shadow-sm text-slate-600"
                                                value={ev.role || '一般'}
                            ...
                            {/* The "Insert" button in between events */}
                            {idx < timelineEvents.length - 1 && (
                                <div className="h-4 flex items-center justify-center -ml-8 my-2 relative z-10 w-full group/add">
                                    <button onClick={() => addEvent(idx)} className="bg-white border text-blue-500 rounded-full p-1 opacity-0 group-hover/add:opacity-100 hover:bg-blue-50 shadow-sm transition-opacity absolute">
              ...
              <div className="mt-8 space-y-4 pt-4 border-t">
                  <div className="bg-blue-50/50 border border-blue-100 rounded-lg p-3">
                     <div className="flex justify-between items-center mb-2">
                        <h4 className="font-bold text-sm text-blue-800">私用外出 (中抜け等 別枠)</h4>
                        <button type="button" onClick={() => setPersonalOuts([...personalOuts, { start_time: '', end_time: '' }])} className="text-xs bg-white text-blue-600 border border-blue-200 px-2 py-1 rounded shadow-sm hover:bg-blue-50 flex gap-1"><Plus className="w-3 h-3"/> 追加</button>
                     </div>
                     <div className="space-y-2">
                        {personalOuts.map((out, idx) => (
                           <div key={idx} className="flex gap-2 items-center">
                              <input type="time" value={out.start_time} onChange={(e) => { const a = [...personalOuts]; a[idx].start_time = e.target.value; setPersonalOuts(a); }} className="w-32 h-8 border rounded px-2" />
                              <span className="text-muted-foreground">〜</span>
                              <input type="time" value={out.end_time} onChange={(e) => { const a = [...personalOuts]; a[idx].end_time = e.target.value; setPersonalOuts(a); }} className="w-32 h-8 border rounded px-2" />
                              <button onClick={() => { const a = [...personalOuts]; a.splice(idx, 1); setPersonalOuts(a); }} className="text-red-400 hover:text-red-500"><Trash2 className="w-4 h-4"/></button>
                           </div>
                        ))}
                     </div>
                  </div>
                  ...
                  <div>
                    <label className="text-sm font-bold text-slate-700 block mb-1">備考</label>
                    <textarea value={memo} onChange={(e) => setMemo(e.target.value)} className="w-full h-16 border rounded-md p-2 text-sm resize-none" placeholder="遅刻、早退の理由など"/>
                  </div>
              </div>
            ...
            <div className="p-4 bg-white border-t flex justify-end gap-3 shrink-0">
              <button onClick={() => setIsModalOpen(false)} className="border bg-white hover:bg-slate-100 h-10 px-4 rounded-md font-medium text-sm">キャンセル</button>
              <button onClick={saveRecord} className="bg-blue-600 text-white hover:bg-blue-700 shadow-sm h-10 px-6 rounded-md font-medium text-sm">保存する</button>
            </div>
          </div>
        </div>
      )}
...
console.log('WorkerAttendance.tsx rewritten successfully!');
