...
async function migrate() {
  ...
  // Get mappings
  const { data: vehiclesData } = await supabase.from('vehicle_master').select('id, vehicle_name')
  const { data: workersData } = await supabase.from('worker_master').select('id, name')

  const vehicleMap = new Map()
  vehiclesData.forEach(v => vehicleMap.set(v.vehicle_name.replace(/"/g, '').trim(), v.id))

  const workerMap = new Map()
  workersData.forEach(w => workerMap.set(w.name.replace(/"/g, '').trim().replace(/　/g, ' '), w.id)) // replace full width space
  ...
  for (const row of records) {
    ...
    const vName = vNameRaw ? vNameRaw.split('-')[0] : null // like ジムニー-1021 -> ジムニー
    ...
    const currentMileage = row['今回の装甲距離'] ? parseInt(row['今回の装甲距離'], 10) : null
    
    // Note: the CSV header says "前回の装甲距離" and "今回の装甲距離" - note the typo "装甲".
    const payload = {
    ...
    // Only insert if mapped vehicle correctly, or maybe insert anyway with null? Better to only insert if we matched vehicle.
    if (vehicleId) {
  ...
  if (inserts.length > 0) {
      const { error } = await supabase.from('vehicle_inspections').insert(inserts)
      if (error) {
