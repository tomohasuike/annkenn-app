...
// 対象設定
const SOURCE_PDF_PATH = '/Users/hasuiketomoo/Downloads/catalog_densetsu-kai.pdf';
const MANUFACTURER = 'ネグロス電工';
const CATALOG_NAME = '電設一般カタログ';
const START_PAGE = 1;
...
async function processCatalog() {
  ...
  // 進捗保存ファイル
  const progressFile = path.resolve('split_progress.json');
  let currentProgress = {};
  if (fs.existsSync(progressFile)) {
      currentProgress = JSON.parse(fs.readFileSync(progressFile));
  }
  ...
  for (let pageNum = START_PAGE; pageNum <= totalPages; pageNum++) {
      ...
      try {
          ...
          if (dbErr) {
              ...
              fs.writeFileSync(progressFile, JSON.stringify(currentProgress));
          }
