...
async function migrateBillingData() {
  ...
  for (const row of invoicesRecords) {
    // Map legacy comma-separated IDs to Supabase UUID array
    const legacyProjectIds = (row['対象工事リスト'] || '').split(',').map(s => s.trim()).filter(Boolean);
    let masterProjectId = null; // We use the first valid one for project_id column
    // The previous implementation used project_id column as text (legacy UUID string), but wait...
    // Let's check invoices table schema
    // In our App, project_id is UUID.
    ...
    // We allow setting project_id to null or a valid UUID. If no valid UUID, we fallback to a special project or just null.
    // Assuming 'vacation' project uuid is an option but it's for schedule. For invoices we can leave project_id null if allowed
    // Let's just use masterProjectId.
    // Ensure idempotency
    ...
    const { data: insertedInv, error: invError } = await supabase
      ...
      .insert({
        ...
        project_id: masterProjectId // This handles the foreign key
      })
      .select();
